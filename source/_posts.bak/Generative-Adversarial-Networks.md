...
答案就是：
为训练生成器，辨别器不得丝告诉生成器如何���整从而使它生成的图片变徟更加眝实。
生樐器必鱻向辨别器寻求建誮！
直观杤说，辨别器告诉生成器每个像素应调整多少来使整幅图僟f��真实一点点。
技术上�ݥ说，通过反向传播辨别ř�输出的梯度来ః整生成哾片。令还燍方式���练生���器，你将会得到与图片ὢ状一样的梯度向量も
夂柜你把这些梯度加到生成的图片上，��辠别器看�ݥ，图片吱会变�>�更真实一犹。
但是樑们不仅仅把梯度加ሱ图片d��。g��反，我们进一���反��传播这些图片梯度成为组我唟成器的权重，这样一来���生成器山e��习刲如佔生成这幅Ɩ�图則。
我重复渀遍，为成���的图片你必须向老师屝社佤的e��䭜，得到反馈！
如果辨别器d��帮助疟成器的话，那就太残酷了，因为生成器实际做的工�=�比螨别器更艰难，它生战图片！
这報是生成器如何��训练的。
就像这栗，来嚖训练生成器和辨别器，直到达到一个平衡状态。
